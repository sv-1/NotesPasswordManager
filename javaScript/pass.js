...
function FetchAllValuesDisplayTable() {
    ...
    for (var i = 0; i < arrayFormated.length; i++) {
        output += `
            ...
            <td><button class="btn btn-danger" onclick="deletePassword(${i})">Delete</button></td>
            </tr>
        `;
    }
...
function deletePassword(index) {
    let allstoredvaluesinArrayForm = JSON.parse(localStorage.getItem('hold'));
    
    // Remove the password at the given index
    allstoredvaluesinArrayForm.splice(index, 1);
